 ...
 *     - user interface using FlowPane
 *     - Add the instances of 3 Buttons to pane1 created by FlowPane and other 3
 *       instances of Buttons using FlowPane to pane2
 *       - scene1 for pane1 with a specific size
 *       - scene2 for pane2 with a different size
 *     - Set different titles to two stages and display two stages
 *     - The output should look like the screen below(see problems)
 *     - Provide appropriate Java comments
 *
 * Assumptions:
 *   - Buttons will only print that they were clicked - when clicked
 *   - Sperate object, but same class will need to be used for the stages
 *   - Java comments just means "comment your code" - not javadoc
 *   - The picture states that the title is SampleFlowPanel
 *     - It was low quality
 *   - FlowPaneDemo will not be the name of the main class
 *     - Global assignment directions overwrite specific assignment directions
 *       unless explicetly specified
 *
 * Pseudocode:
 *   1. in the start method:
 *   1.1 create a second stage and two panes
 *   1.2 set titles for the stages
 *   1.3 create 6 buttons
 *   1.4 set button titles and actions(print)
 *   1.5 add buttons to pane
 *   1.6 create scenes from panes and add scenes to stages
 *   1.7 show both stages
 */
