package Unit_02.Problem_Book_07_05;
...
/*
 * Description: Write a program that reads in ten numbers and displays the
 * number of distinct numbers and the distinct numbers separated by exactly one
 ...
 * distinct numbers. Here is the sample run of the program:
 *
 * Assumptions: - Numbers may(or may not) be all on one line - Numbers may be
 * doubles
 *
 * Pseudocode: 1. ask user for ten numbers 2. create array(doubles), int
 * counter=0 3. read in line 4. while counter < 10 4.1 split line by non-numbers
 * 4.2 for each number(see step 4.1)(also exit if counter >= 10) 4.2.1 add to
 * array(if not there) 4.2.2 increase counter 5. print array space separated
 */
...
public class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    while(nNums < NUMBER_OF_INPUTS) {// this loop does work per line entered
      ...
      String[] astrNums = input.readLine().split("[^0-9\\.]+");// split line
                                                               // into
                                                               // strings of
                                                               // nums
      for(int i = 0; i < astrNums.length && nNums < NUMBER_OF_INPUTS; i++) {
        if(!inList(lstrOut, astrNums[i])) // for each num in line, ignoring
                                          // those
