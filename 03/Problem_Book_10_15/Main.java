...
/* Description:
 *   (Geometry: the bounding rectangle) A bounding rectangle is the
 ...
 *     test program that prompts the user to enter five points and displays
 *     the bounding rectangle’s center, width, and height. Here is
 *     a sample run:
 *   ```
 ...
 *
 * Assumptions:
 *   - I guess I'll have to do problem 10.13 as well
 *     - However, don't expect me to type that up as well
 *   - Both classes(tester and rectagle2d) will be in the same file
 *     - For [my] convenience
 *   - People may not follow direcions: there may be more than 5 points
 *
 * Pseudocode:
 *   1. Find min and max x/y values
 *   2. COnstruct rect using: (minx, miny, maxx-minx, maxy-miny)
 */
...
class MyRectangle2D {
  ...
  public boolean contains(MyRectangle2D rect) {
    return this.contains(rect.getX(), rect.getY()) &&
      this.contains(rect.getX()+rect.getWidth(), rect.getY()+rect.getHeight());
  }
...
public class Main {
  public static void main(String[] args) throws IOException {
    ...
    double[][] points = new double[nums.length/2][2];//create array of points...
                                                     //see assumptions
